...
| 28     | _(null)_     | —                                | Unsupported                                                                                                                                                                                                                                                                                                                        |
| 29     | `0x0040904c` | _(no mnemonic)_                  | ClearDiagnosticInformation (0x14) with `0xFFFFFF` mask: operand 0 = single attempt, operand 1 = up to 4 retries with ECUReset between each                                                                                                                                                                                         |
| 30     | `0x004127ea` | _(no mnemonic)_                  | Internal — OR operand byte into `context+0x22` (VM execution flags)                                                                                                                                                                                                                                                                |
| 31     | `0x00412867` | _(no mnemonic)_                  | Internal — AND NOT operand byte into `context+0x22`                                                                                                                                                                                                                                                                                |
| 32     | `0x00411d47` | _(no mnemonic)_                  | ReadDataByIdentifier (0x22) DID `0xF100`; reads flash count; compares against limit in `DAT_00651408[operand×3]`; operand 0–2 selects limit entry, operand ≥3 → error `0x120000`; over-limit → error `0x270000`                                                                                                                    |
| 33     | _(null)_     | —                                | Unsupported                                                                                                                                                                                                                                                                                                                        |
| 34     | _(null)_     | —                                | Unsupported                                                                                                                                                                                                                                                                                                                        |
| 35     | `0x004067f5` | _(no mnemonic)_                  | IOCBI — enables sub-ECU power rail; searches table `DAT_0043c220` (up to 3 entries) for an entry whose node ID matches `context+2`; calls `uds_channel_power_rail_io_control` on the matched entry; returns error `0x1a` if no match found                                                                                         |
| 36     | `0x00412cc5` | _(no mnemonic)_                  | ReadDataByIdentifier (0x22), DIDs `0x561`–`0x563` → writes intrusion sensor serial/compensation/part to `modinfo.log`                                                                                                                                                                                                              |
| 37     | `0x00411609` | _(no mnemonic)_                  | RoutineControl (0x31) routine `0x601`: start (subfunction 1), then polls requestResults (subfunction 3) up to 50 times (100 ms apart) until response byte == 1; skipped if firmware check gate (`FUN_0040f0ea(0x43)`) returns 0                                                                                                    |
| 38     | `0x004115a2` | _(no mnemonic)_                  | Stub — always returns 0                                                                                                                                                                                                                                                                                                            |
| 39     | `0x00412ec6` | _(no mnemonic)_                  | RoutineControl (0x31): routine ID from `DAT_006515b4[operand×2]` (operand 0 or 1 only; ≥2 → error `0x120000`); subfunction byte from `context+0x29`; validates response (operand 0 only): session `0x01` → error if response byte == `0x01`; session `0x03` → error if response byte != `0x00`; error `0x250000` on failure        |
| 40     | `0x004092c6` | _(no mnemonic)_                  | Internal — OR bit 0 into `context+0x22`; returns `0x230000` if operand == 1, else 0                                                                                                                                                                                                                                                |
| 41+    | _(null)_     | —                                | End of table                                                                                                                                                                                                                                                                                                                       |
